    ...
    public partial class AltoCPU
    {
        ...
        public abstract class Task
        {
            ...
            protected virtual InstructionCompletion ExecuteInstruction(MicroInstruction instruction)
            {
                ...
                if (_rdRam)
                {
                    _busData &= _cpu.UCodeMemory.ReadRAM();
                    _rdRam = false;
                }
                ...
                {
                    aluData = _cpu.ALU.Execute(instruction.ALUF, _busData, _cpu._t, _skip);
                }
                else
                {
                    aluData = _busData;
                ...
                if (_wrtRam)
                {
                    _cpu.UCodeMemory.WriteRAM(aluData, _cpu._m);
                    _wrtRam = false;
                }
                ...
                switch (instruction.F1)
                {
                    ...
                    case SpecialFunction1.LLSH1:
                        _cpu.Shifter.SetOperation(ShifterOp.ShiftLeft);
                        break;

                    case SpecialFunction1.LRSH1:
                        _cpu.Shifter.SetOperation(ShifterOp.ShiftRight);
                        break;

                    case SpecialFunction1.LLCY8:
                        _cpu.Shifter.SetOperation(ShifterOp.RotateLeft);
                        break;
                ...
                if (_loadR || instruction.NeedShifterOutput)
                {
                    ...
                    if (_cpu.Shifter.Op == ShifterOp.None)
                    {
                        _cpu.Shifter.Output = _cpu._l;
                    }
                    else
                    {
                        _cpu.Shifter.DoOperation(_cpu._l, _cpu._t);
                    }
                }
                ...
                switch(instruction.F2)
                {
                    case SpecialFunction2.ShLt0:
                        ...
                        //
                        if ((short)_cpu.Shifter.Output < 0)
                        {
                            _nextModifier |= 1;
                        }
                        break;
                ...
                if (_loadR)
                {
                    _cpu._r[_rSelect] = _cpu.Shifter.Output;
                }
                ...
                if (instruction.LoadT)
                {
                    ...
                    // from its source."
                    //
                    _cpu.UCodeMemory.LoadControlRAMAddress(aluData);
                }
                ...
                if (instruction.LoadL)
                {
                    ...
                    _cpu._aluC0 = (ushort)_cpu.ALU.Carry;
                }
